...
class TaskRepositoryInternalImpl extends SimpleR2dbcRepository<Task, Long> implements TaskRepositoryInternal {
    ...
    @Override
    public Mono<Task> findOneWithEagerRelationships(Long id) {
        return findById(id);
    }

    @Override
    public Flux<Task> findAllWithEagerRelationships() {
        return findAll();
    }

    @Override
    public Flux<Task> findAllWithEagerRelationships(Pageable page) {
